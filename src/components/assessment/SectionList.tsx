  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2">
      {ASSESSMENT_SECTIONS.map((s) => {
        const Icon =
          (Icons[s.icon as keyof typeof Icons] as Icons.LucideIcon) ?? Icons.CircleDot;
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                ...
                {disabled ? <Badge variant="outline" className="text-[10px]">Coming soon</Badge> : null}
              </div>
              <p className="mt-0.5 text-xs text-muted-foreground">{s.description}</p>
              <div className="mt-2 flex items-center gap-3 text-[11px] text-muted-foreground">
                <span className="inline-flex items-center gap-1"><Clock className="h-3 w-3" /> ~{s.estimated_minutes} min</span>
